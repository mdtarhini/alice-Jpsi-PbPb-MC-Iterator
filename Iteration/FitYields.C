...
//--------------------------------------------------------------------------------------------//
void FitPtYield(int centMin = 0, int centMax = 10, Int_t iteration = 1)
{
  PrintMessage("Fitting pt yield", "-");
  TCanvas *canYieldPt = new TCanvas(Form("canYieldPt_%d_Cent-%dto%d", iteration, centMin, centMax), "", 900, 800);
  ...
  do
  {
    ...
    fitResultPtr = histoYiledVsPt->Fit(fPtDistribution, "SRIQ", "", ptFitRangeMin, ptFitRangeMax);
...
//--------------------------------------------------------------------------------------------//
void FitRapYield(int centMin = 0, int centMax = 10, Int_t iteration = 1)
{
  PrintMessage("Fitting y yield", "-");

  TCanvas *canYieldRap = new TCanvas(Form("canYieldRap_%d_Cent-%dto%d", iteration, centMin, centMax), "", 900, 800);
  ...
  do
  {
    ...
    fitResultPtr = histoYiledVsRapidity->Fit(fRapDistribution, "RISQ", "", rapFitRangeMin, rapFitRangeMax);
